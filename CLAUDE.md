# CLAUDE.md — claudepilot

Read this first. It exists so you can evolve claudepilot **without re-scanning
...
```bash
tmux kill-session -t claudepilot-server 2>/dev/null; sleep 1
tmux new-session -d -s claudepilot-server \
  "cd ~/projects/claudepilot && CLAUDE_CODE_OAUTH_TOKEN=\$(security find-generic-password -s 'Claude Code-credentials' -a \"\$USER\" -w | jq -r '.claudeAiOauth.accessToken') node dist/server.js > /tmp/cp.log 2>&1"
```
...
| `src/session.ts` | `ClaudePilot` — drives `claude` in a **node-pty** PTY + `@xterm/headless`. Dies with the server. |
| `src/tmux.ts` | `TmuxPilot` — same interface as `ClaudePilot`, but runs `claude` in a **detached tmux session** (`cp-<sessionId>`). **Survives server restart** (reattaches). Default transport when tmux is present. |
...
| `.claude/skills/claudepilot-agents/pilotctl.mjs` | Thin client that lets an agent spawn/pilot other agents through the server (used by the `claudepilot-agents` skill). |
...
- **Sessions outlive clients**: closing a tab detaches; the process keeps
  running and is reclaimed only after `CLAUDEPILOT_IDLE_MIN` min (default 60)
...
2. **Detection heuristics are fragile.** `screenShowsWork` must ignore a
   *quoted* "esc to interrupt" (Claude explaining claudepilot tripped it →
...
4. **The resume-from-summary prompt is auto-answered** ("full session as-is")
   at startup and never surfaced (`CLAUDEPILOT_RESUME_SUMMARY=1` to disable).
